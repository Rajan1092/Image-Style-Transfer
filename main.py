...
def upload_image():
    ...
    # Convert the final target image to a format suitable for web display
    result_image = im_convert(target)
    result_image_pil = Image.fromarray((result_image * 255).astype(np.uint8))
    
    # Save to an in-memory file and return it directly
    img_io = BytesIO()
    result_image_pil.save(img_io, 'JPEG')
    img_io.seek(0)

    return send_file(img_io, mimetype='image/jpeg')
 # Ensure URL points to static file
